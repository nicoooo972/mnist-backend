...
class TestModelQuality:
    ...
    def test_model_accuracy_threshold(self):
        ...
        assert accuracy >= min_accuracy, f"Accuracy {accuracy}% < minimum {min_accuracy}%"
    ...
    def test_model_inference_speed(self, trained_model_data):
        ...
        assert inference_time < max_inference_time, f"Inference too slow: {inference_time:.3f}s"
    ...
    def test_model_confidence_calibration(self, trained_model_data):
        ...
        assert mean_confidence > 0.3, f"Modèle pas assez confiant: {mean_confidence:.3f}"
    ...
    def test_model_class_balance_performance(self, trained_model_data):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            class_accuracies = class_correct / (class_total + 1e-8)  # Éviter division par 0
            ...
            for i in range(10):
                if class_total[i] > 0:  # Seulement si la classe est présente
                    assert class_accuracies[i] >= min_class_accuracy, \
...
class TestModelInvariance:
    ...
    def test_batch_size_invariance(self, model_and_data):
        ...
        assert torch.allclose(individual_batch, full_batch_output, atol=1e-6), \
...
class TestModelSafety:
    ...
    def test_model_input_validation(self):
        ...
        with torch.no_grad():
            ...
            assert not torch.isnan(output_extreme).any(), "Modèle produit NaN avec valeurs extrêmes"
            assert not torch.isinf(output_extreme).any(), "Modèle produit Inf avec valeurs extrêmes"
